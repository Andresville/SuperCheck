    ...
    const addProductToCart = (product) => {
        ...
        if (!isInCart) {
            ...
            setQuantityCart(product.quantity)
            ...
            localStorage.setItem('quantityCart', quantityCart + product.quantity)
            ...
            if (refreshQuantity.quantity + product.quantity <= product.stock) {
                ...
                setQuantityCart(quantityCart + product.quantity)
                localStorage.setItem('quantityCart', quantityCart + product.quantity)
    ...
    const deleteProduct = (prod) => {
        ...
        setQuantityCart(quantityCart - prod.quantity)
        localStorage.setItem('quantityCart', quantityCart - prod.quantity)
    };

    const deleteAll = () => {
        setCartListItems([]);
        localStorage.setItem('cartListItems',[]);
        setTotalPrice(0);
